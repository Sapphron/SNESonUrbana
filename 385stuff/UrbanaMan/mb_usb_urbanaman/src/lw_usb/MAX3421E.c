...
//	XSpi_SetSlaveSelectReg(InstancePtr, Mask)
// InstancePtr points to the object of the instance port? soooo the slave/peripheral?
int i;
/* Functions    */
/* Single host register write   */
void MAXreg_wr(BYTE reg, BYTE val) {
	//psuedocode:
	//xil_printf("MAXreg_wr reg: %u \n", reg);
	XSpi_SetSlaveSelect(&SpiInstance, 1); //select MAX3421E //this may be wrong, should be XSpi Set Slave Select Reg
	//write reg + 2 via SPI
	u8 SendBuff[2];
	SendBuff[0] = reg + 2; // might make SendBuff a 16 bit pointer?
	//write val via SPI
	SendBuff[1] = val;
	int return_code = XSpi_Transfer(&SpiInstance, SendBuff, NULL, 2);
	//read return code from SPI peripheral (see Xilinx examples)
	//if return code != 0 print an error
	if(return_code != 0){
		xil_printf("error in MAXreg_wr function");
	}
	//xil_printf("MAXreg_wr, %d \n", GetUsbTaskState());
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int return_code = XSpi_Transfer(&SpiInstance, SendBuff, NULL, nbytes+1);
	//if return code != 0 print an error
	if(return_code != 0){
		xil_printf("error in MAXbytes_wr");
	}
//	xil_printf("MAXbytes_wr \n");
	XSpi_SetSlaveSelect(&SpiInstance, 0);//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	return (data+nbytes);
}
...
BYTE MAXreg_rd(BYTE reg) {
	//psuedocode:
	//xil_printf("MAXreg_rd reg: %u \n", reg);
	XSpi_SetSlaveSelect(&SpiInstance, 1);//select MAX3421E (
	//write reg via SPI
	u8 SendBuff[2];
	SendBuff[0] = reg;
	SendBuff[1] = 0;
	int return_code = XSpi_Transfer(&SpiInstance, SendBuff, SendBuff, 2);
	//read return code from SPI peripheral
	//if return code != 0 print an error
	if(return_code != 0){
			xil_printf("error in MAXreg_rd function");
	}
	//xil_printf("MAXreg_rd, %d \n", GetUsbTaskState());
	XSpi_SetSlaveSelect(&SpiInstance, 0);//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	return SendBuff[1];
}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	//psuedocode:
	//xil_printf("AOOOAOAOOAOAAOOA");
	XSpi_SetSlaveSelect(&SpiInstance, 1);//select MAX3421E (may not be necessary if you are using SPI peripheral)
	//write reg via SPI
	xil_printf("MAXbytes_rd reg: %u \n",reg);
	u8 SendBuff[nbytes+1];
	SendBuff[0] = reg;
	//read data[n] from SPI, where n goes from 0 to nbytes-1
	//read return code from SPI peripheral
	int return_code = XSpi_Transfer(&SpiInstance, SendBuff, SendBuff, nbytes+1);
	for(i = 0; i<nbytes; i++){
		data[i] = SendBuff[i+1];
	}
	//if return code != 0 print an error
	...
	//BYTE* return_val = data + nbytes;
	XSpi_SetSlaveSelect(&SpiInstance, 0); //deselect MAX3421E (may not be necessary if you are using SPI peripheral)
//	xil_printf("data: %u ",data);
//	xil_printf("nbytes: %u ",nbytes);
//	xil_printf("data + nbytes: %u \n", (data+nbytes));
	return (data+nbytes);
}
