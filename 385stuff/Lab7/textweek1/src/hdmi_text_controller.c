...
void paletteTest()
{
	...
	for (int i = 0; i < 10; i++)
	{
		sleep_MB (1);
		for (int j = 0; j < 8; j++)
			setColorPalette(j, 	rand() % 16, rand() % 16,rand() % 16, rand() % 16, rand() % 16,rand() % 16); //set color 0 to random color;
...
void setColorPalette (uint8_t color, uint8_t red_b, uint8_t green_b, uint8_t blue_b, uint8_t red_a, uint8_t green_a, uint8_t blue_a)
{
	hdmi_ctrl->colors[color]= ( red_b << 21| green_b << 17| blue_b << 13 | red_a << 9 | green_a << 5 | blue_a << 1);
...
void textHDMIColorScreenSaver()
{
	...
	for (int i = 0; i < 8; i++)
	{
		int color_odd = (2*i)+1;
		int color_even = (2*i);
		setColorPalette (i, colors[color_odd].red, colors[color_odd].green, colors[color_odd].blue, colors[color_even].red, colors[color_even].green, colors[color_even].blue);
	}
	while (1)
	{
		fg = rand() % 16;
