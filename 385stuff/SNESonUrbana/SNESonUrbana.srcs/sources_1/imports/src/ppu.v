...
blk_mem_gen_cgram cgram (
  .clka(CLK),    // input wire clka
  .ena(1'b1),      // input wire ena
  .wea(CGRAM_WE),      // input wire [0 : 0] wea
  .addra(CGRAM_ADDR),  // input wire [7 : 0] addra
  .dina(CGRAM_Q),    // input wire [14 : 0] dina
  .douta(douta),  // output wire [14 : 0] douta
  .clkb(CLK),    // input wire clkb
  .enb(1'b1),      // input wire enb
  .web(~RST_N),      // input wire [0 : 0] web
  .addrb(CGRAM_ADDR_CLR),  // input wire [7 : 0] addrb
  .dinb({CGRAM_ADDR_CLR[6:0], CGRAM_ADDR_CLR}),    // input wire [14 : 0] dinb
  .doutb()  // output wire [14 : 0] doutb
);
...
blk_mem_gen_oam OAM(
  .clka(CLK),    // input wire clka
  .ena(1'b1),      // input wire ena
  .wea(OAM_WE),      // input wire [0 : 0] wea
  .addra(OAM_ADDR_A),  // input wire [7 : 0] addra
  .dina(OAM_D),    // input wire [15 : 0] dina
  .douta(OAMIO_Q),  // output wire [15 : 0] douta
  .clkb(CLK),    // input wire clkb
  .enb(1'b1),      // input wire enb
  .web(1'b0),      // input wire [0 : 0] web
  .addrb(OAM_ADDR_B),  // input wire [6 : 0] addrb
  .dinb(32'b0),    // input wire [31 : 0] dinb
  .doutb(OAM_Q)  // output wire [31 : 0] doutb
);
